...
/**
 * Class for SpellCheckDecorator, implements AbstractTextDecorator.
 *
...
public class SpellCheckDecorator extends AbstractTextDecorator {
    ...
    Queue<String> misspelledWords;
    private final MyLogger myLogger = MyLogger.getInstance();

    /**
     * Constructor for SpellCheckDecorator
     * @param adtIn Abstract Data type Sentence decorator
     * @param idIn Instance of inputDetails class
     */
    public SpellCheckDecorator(AbstractTextDecorator adtIn, InputDetails idIn) {
        adt = adtIn;
        id = idIn;
        myLogger.writeMessage("Constructor of SpellCheck Decorator was called.", MyLogger.DebugLevel.SPELLCHECKER_DECORATOR);
    ...
    private List<Integer> checkCompleteWord(String inputString, String word, int start) {
        ...
        Pattern pattern2 = Pattern.compile("\\b" + "(?i)" + Prefixes.KEYWORD.getPrefix() + word + Suffixes.KEYWORD.getSuffix() + "\\b");
        Pattern pattern3 = Pattern.compile("\\b" + "(?i)" + Prefixes.KEYWORD.getPrefix() + Prefixes.MOST_FREQUENT.getPrefix() +
                word + Suffixes.MOST_FREQUENT.getSuffix() + Suffixes.KEYWORD.getSuffix() + "\\b");
        ...
        if (matcher.find(start)) {
            list.add(matcher.start());
            list.add(matcher.start() + word.length() + Prefixes.KEYWORD.getPrefix().length() + Suffixes.KEYWORD.getSuffix().length());
            return list;
        }
        matcher = pattern3.matcher(inputString);
        if (matcher.find(start)) {
            list.add(matcher.start());
            list.add(matcher.start() + word.length() + Prefixes.MOST_FREQUENT.getPrefix().length() + Suffixes.MOST_FREQUENT.getSuffix().length() +
                    Prefixes.KEYWORD.getPrefix().length() + Suffixes.KEYWORD.getSuffix().length());
            return list;
        }
    ...
    private StringBuilder replaceAllOccurrence(String inputString, String word) {
        ...
        while (start < inputString.length()) {
            if (start == -1)
                break;
            endIndex = list.get(1);
            inputString = inputString.substring(0, start) + Prefixes.SPELLCHECK.getPrefix() + inputString.substring(start, endIndex) + Suffixes.SPELLCHECK.getSuffix() + inputString.substring(endIndex);
    ...
    public void processInputDetails() {
        ...
        String misspelledWordsFromFile = id.readMisspelledWords().toString();
        misspelledWords = new LinkedList<>(Arrays.asList(misspelledWordsFromFile.split("\\s")));
        myLogger.writeMessage("The contents of Misspelled Words File were retrieved into a data structure.", MyLogger.DebugLevel.SPELLCHECKER_DECORATOR);
        for (String misspelledWord : misspelledWords) {
            StringBuilder stringBuilderMisspelledWords = replaceAllOccurrence(inputString, misspelledWord);
            id.storeInput(stringBuilderMisspelledWords);
            inputString = id.getStringBuilderInput().toString();
            myLogger.writeMessage("The misspelled word: \"" + misspelledWord + "\" was replaced with the respective tags.", MyLogger.DebugLevel.SPELLCHECKER_DECORATOR);
        }
        if (adt != null) {
            myLogger.writeMessage("Proceeding with the next Decorator: " + adt.toString(), MyLogger.DebugLevel.SPELLCHECKER_DECORATOR);
